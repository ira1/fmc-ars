...
#
    ...
    colexpr = "coalesce(round(100.0*sum(case compose_inc_direction when 501 then null when 404 then null else compose_inc_direction end) / nullif(count (nullif (compose_inc_direction, 501)),0) ,1),0) as composing,
coalesce(round(100.0*count(case compose_inc_direction when 501 then null else compose_inc_direction end) / nullif(count (*),0) ,1),0) as pct_composing,
coalesce(round(100.0*sum(case record_inc_direction when 501 then null when 404 then null else record_inc_direction end) / nullif(count (nullif (record_inc_direction, 501)),0)  ,1),0) as recording,
coalesce(round(100.0*count(case record_inc_direction when 501 then null else record_inc_direction end)/ nullif(count (*),0) ,1),0) as pct_record,
coalesce(round(100.0*sum(case salary_inc_direction when 501 then null when 404 then null else salary_inc_direction end) / nullif(count (nullif (salary_inc_direction, 501)),0)  ,1),0) as salaried,
coalesce(round(100.0*count(case salary_inc_direction when 501 then null else salary_inc_direction end)/ nullif(count (*),0)  ,1),0) as pct_salary,
coalesce(round(100.0*sum(case perform_inc_direction when 501 then null when 404 then null else perform_inc_direction end) / nullif(count (nullif (perform_inc_direction, 501)),0) ,1),0) as performing,
coalesce(round(100.0*count(case perform_inc_direction when 501 then null else perform_inc_direction end) / nullif(count (*),0) ,1),0) as pct_perform,
coalesce(round(100.0*sum(case session_inc_direction when 501 then null when 404 then null else session_inc_direction end) / nullif(count (nullif (session_inc_direction, 501)),0) ,1),0) as sessions,
coalesce(round(100.0*count(case session_inc_direction when 501 then null else session_inc_direction end) / nullif(count (*),0) ,1),0) as pct_session,
coalesce(round(100.0*sum(case merch_inc_direction when 501 then null when 404 then null else merch_inc_direction end) / nullif(count (nullif (merch_inc_direction, 501)),0) ,1),0) as merch,
coalesce(round(100.0*count(case merch_inc_direction when 501 then null else merch_inc_direction end) / nullif(count (*),0) ,1),0) as pct_merch,
coalesce(round(100.0*sum(case teach_inc_direction when 501 then null when 404 then null else teach_inc_direction end) / nullif(count (nullif (teach_inc_direction, 501)),0) ,1),0) as teaching,
coalesce(round(100.0*count(case teach_inc_direction when 501 then null else merch_inc_direction end) / nullif(count (*),0) ,1),0) as pct_teach"
